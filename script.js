...
if (mobileMenuButton && navLinks && mobileMenuIcon) {
  ...
  on(
    ...
    true,
  ) // Use capture phase if needed, but usually bubble is fine
}
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const commonChartOptions = {
      ...
      plugins: {
        ...
        annotation: {
          // Ensure annotation plugin options are always present
          annotations: {},
        },
      },
      scales: {
    ...
    const createAnnotationLabel = (xVal, yVal, content, yAdj = -15, xAdj = 0) => ({
      ...
      content: content,
      color: mutedColor,
      font: { size: window.innerWidth <= 768 ? 9 : 10, weight: "600" },
      position: "start",
      yAdjust: yAdj,
      xAdjust: xAdj,
    ...
    const loadAnalysisData = async (ticker) => {
      ...
      try {
        ...
        if (arCtx) {
          ...
          console.warn("AR chart canvas not found")
        }
        ...
        chartsToResize.forEach((chart, index) => {
          ...
          try {
            ...
            if (chart.options.plugins?.tooltip?.bodyFont)
              chart.options.plugins.tooltip.bodyFont.size = isMobile ? 11 : 12
...
// Add this to the existing script.js file, at the end of the file

// Handle search toggle functionality
document.addEventListener("DOMContentLoaded", () => {
  // This functionality is now in search-toggle.js
  // The separate file ensures it's loaded on all pages
})
