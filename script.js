...
const populateTable = (tbodyId, tableRowData) => {
  ...
  tableRowData.forEach((row) => {
    ...
    tr.innerHTML = `
            <td data-label="Factor">${row.factor ? `<span>${row.factor}</span>` : `<span>-</span>`}</td>
            <td data-label="Opportunities">${row.opportunities ? `<span>${row.opportunities}</span>` : `<span>-</span>`}</td>
            <td data-label="Risks">${row.risks ? `<span>${row.risks}</span>` : `<span>-</span>`}</td>
        `
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const loadAnalysisData = async (ticker) => {
      ...
      try {
        ...
        const originalRevenueGrowth = chartData.revenueGrowth || []
        const originalArGrowth = chartData.arGrowth || []
        const originalCfoGrowth = chartData.cfoGrowth || []
        const originalNiGrowth = chartData.niGrowth || []
        ...
        if (revenueCtx && displayRevenueGrowth.length > 0) {
          // Check if data exists after potential slicing
          try {
        ...
        if (arCtx && displayRevenueGrowth.length > 0 && displayArGrowth.length > 0) {
          ...
          try {
            ...
            // Note: Annotations from JSON are based on original data indices.
            // If data is sliced, annotations might point to non-existent data points or wrong years.
            // A more robust solution would involve mapping annotations to the sliced data,
            // but keeping it simple as requested, we'll add them as they are.
            ...
            if (
              ...
              chartData.annotations.arChart.forEach((anno, index) => {
                // Check if the annotation's xValue (index) exists in the *displayed* data
                if (
                  typeof anno.xVal === "number" &&
                  anno.xVal >= 0 &&
                  anno.xVal < displayLabels.length &&
                  typeof anno.yVal === "number" &&
                  anno.content
                ) {
                  // Adjust xVal if data was sliced
                  const adjustedXVal =
                    isMobile && originalLabels.length > MAX_DATA_POINTS_MOBILE
                      ? Math.max(0, anno.xVal - (originalLabels.length - MAX_DATA_POINTS_MOBILE))
                      : anno.xVal

                  // Only add if the adjusted index is valid for the displayed data
                  if (adjustedXVal >= 0 && adjustedXVal < displayLabels.length) {
                    ...
                    console.warn(
                      `AR Annotation ${index + 1} skipped: Original index ${anno.xVal} is outside the range of displayed mobile data.`,
                    )
                  }
                } else {
            ...
            arChartOptions.plugins.legend.labels.generateLabels = (chart) => {
              ...
              // Filter out the dummy 'Divergence' dataset if no points are highlighted (based on displayed data)
              ...
              data: {
                ...
                datasets: [
                  ...
                  },
                  {
                    ...
                    borderWidth: 2,
                    tension: 0.4,
                    // Use recalculated divergence indices
                    pointRadius: (context) => (arDivergenceIndices.includes(context.dataIndex) ? tinyPointRadius : 0),
                    pointHoverRadius: (context) =>
                      arDivergenceIndices.includes(context.dataIndex) ? tinyPointHoverRadius : 0,
        ...
        if (cashFlowCtx && displayCfoGrowth.length > 0 && displayNiGrowth.length > 0) {
          ...
          try {
            ...
            // Add annotations (adjusting index if sliced, similar to AR chart)
              ...
              chartData.annotations.cashFlowChart.forEach((anno, index) => {
                if (
                  ...
                  if (adjustedXVal >= 0 && adjustedXVal < displayLabels.length) {
                    ...
                    console.warn(
                      `Cash Flow Annotation ${index + 1} skipped: Original index ${anno.xVal} is outside the range of displayed mobile data.`,
                    )
                  }
                } else {
              ...
              data: {
                ...
                datasets: [
                  ...
                  },
                  {
                    ...
                    pointHoverRadius: (context) =>
                      cfDivergenceIndices.includes(context.dataIndex) ? tinyPointHoverRadius : 0,
